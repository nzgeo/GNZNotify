...
static void AddAlertToListView(HWND hList, const Alert& a) {
    ...
    ListView_SetItemText(hList, row, 2, const_cast<wchar_t*>(wev.c_str()));
    ListView_SetItemText(hList, row, 3, const_cast<wchar_t*>(wkey.c_str()));
    ListView_SetItemText(hList, row, 4, const_cast<wchar_t*>(wsum.c_str()));
}
...
static LRESULT CALLBACK AlertsWndProc(HWND hwnd, UINT msg,
    ...
    case WM_CREATE: {
        ...
        struct { const wchar_t* name; int w; } cols[] = {
            { L"ID",       50 }, { L"Time",   162 }, { L"Event",  130 },
            { L"Key",      80 }, { L"Summary", 260 }
        };
        for (int i = 0; i < 5; ++i) {
            LVCOLUMNW lvc{};
